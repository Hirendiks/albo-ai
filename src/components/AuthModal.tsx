...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const [isGoogleGsiReady, setIsGoogleGsiReady] = useState(false);
  const googleBtnRef = useRef<HTMLDivElement>(null);

  const DEFAULT_GOOGLE_CLIENT_ID = "895979960751-qmqknuh60aj6drhrtd7rb6i1ukedgenf.apps.googleusercontent.com";
  ...
  useEffect(() => {
    ...
    // Retry every 200ms up to 15 times while Google script finishes loading
    let attempts = 0;
      ...
      <div className="relative w-full max-w-md my-auto rounded-3xl glass-panel-elevated p-6 sm:p-7 z-10 shadow-2xl border border-white/20 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="mt-5 space-y-3">
          <div className="flex flex-col items-center justify-center min-h-[44px]">
            {/* Google's official iframe button mounts here */}
            <div ref={googleBtnRef} className="flex justify-center w-full" />

            {/* Fallback while Google GIS finishes initializing */}
            {!isGoogleGsiReady && (
              <button
                type="button"
                disabled={isLoading}
                onClick={() => {
                  if (typeof window !== "undefined" && (window as any).google?.accounts?.id) {
                    ...
                    if (input) {
                      input.focus();
                      setErrorMessage("Google Sign-In is loading... or type your Gmail below");
                    }
                  }
                }}
